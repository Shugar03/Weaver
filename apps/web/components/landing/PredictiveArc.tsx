// Predictive Arc (Originkit) adaptado al sistema Weaver + TS borrable.
// Cambios vs original: sin casts `as` (Node strip-only los rechaza),
// preset tipado, uniforms con guard (tsc estricto), resto intacto.
"use client";
...
export type ArchGroup = { peak?: number; archHeight?: number; thickness?: number; falloff?: number };
const ARCH_DEFAULTS: Required<ArchGroup> = { peak: 35, archHeight: 70, thickness: 100, falloff: 250 };

export type PointerGroup = { enabled?: boolean; radius?: number; strength?: number };
const POINTER_DEFAULTS: Required<PointerGroup> = { enabled: true, radius: 220, strength: 60 };

function PredictiveArcBase(props: PredictiveArcProps) {
  const {
    style,
    background = "#030303",
    baseColor = "#FF0000",
  ...
  useEffect(() => {
    ...
    const locCache: Record<string, WebGLUniformLocation | null> = {};
    const mustLoc = (name: string): WebGLUniformLocation => {
      if (!(name in locCache)) locCache[name] = gl.getUniformLocation(prog, name);
      const loc = locCache[name];
      if (!loc) throw new Error(`PredictiveArc: missing uniform ${name}`);
      return loc;
    };
    ...
    const render = (now: number) => {
      ...
      clock = (clock + dt * 0.9 * num(v.speed, 1)) % 6283;
      ...
      const pitchCss = Math.min(bw, bh) / dpr / num(v.density, 160);
      ...
      gl.uniform2f(mustLoc("uRes"), bw, bh);
      gl.uniform1f(mustLoc("uTime"), clock);
      gl.uniform1f(mustLoc("uDpr"), dpr);
      gl.uniform1f(mustLoc("uCell"), Math.max(2, pitchCss * dpr));
      gl.uniform1f(mustLoc("uDot"), pitchCss * 1.2 * num(v.dotSize, 1));
      gl.uniform1f(mustLoc("uPeak"), num(v.peak, 0.35));
      gl.uniform1f(mustLoc("uHeight"), num(v.archHeight, 0.7));
      gl.uniform1f(mustLoc("uThick"), num(v.thickness, 1));
      gl.uniform1f(mustLoc("uFall"), num(v.falloff, 2.5));
      gl.uniform2f(mustLoc("uMouse"), ps.x, ps.y);
      gl.uniform1f(mustLoc("uMouseRadius"), num(v.pointerRadius, 220));
      gl.uniform1f(mustLoc("uMouseStrength"), v.pointerEnabled ? num(v.pointerStrength, 0.6) * ps.active : 0);
      const cg = parseColor(String(v.bg), [0.012, 0.012, 0.012]);
      const cb = parseColor(String(v.base), [0.169, 0.055, 0.369]);
      const ca = parseColor(String(v.accent), [0.627, 0.314, 1.0]);
      const chh = parseColor(String(v.high), [1, 1, 1]);
      gl.uniform3f(mustLoc("uBg"), cg[0], cg[1], cg[2]);
      gl.uniform3f(mustLoc("uBase"), cb[0], cb[1], cb[2]);
      gl.uniform3f(mustLoc("uAccent"), ca[0], ca[1], ca[2]);
      gl.uniform3f(mustLoc("uHigh"), chh[0], chh[1], chh[2]);
...
const WEAVER_PRESET: PredictiveArcProps = {
  background: "#000000",
  baseColor: "#D0FF00",
  accentColor: "#D9FF4D",
  highlight: "#FFFFFF",
  ...
  dotSize: 110,
  speed: 100,
  arch: { peak: 55, archHeight: 60, thickness: 120, falloff: 300 },
  pointer: { enabled: true, radius: 236, strength: 34 },
};

export default function PredictiveArc(props: Partial<PredictiveArcProps>) {
  return <PredictiveArcBase {...WEAVER_PRESET} {...props} />;
}
